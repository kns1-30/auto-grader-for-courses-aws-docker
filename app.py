...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file.filename == '':
            ...
            message ='Incomaptible Extension not allowed, only cc'  
                 
    return render_template('hello.html', var= message)
    
@app.route('/compile/<name>')
def check_file(name):
	...
	retcode = subprocess.call("/usr/bin/g++ uploads/"+name, shell=True)
	if retcode:
	...
	with open('uploads/'+name,'r') as fs:
		var= fs.read()
